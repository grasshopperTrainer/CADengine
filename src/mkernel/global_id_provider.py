...
class GIDP:
    ...
    def get_registered(self, goid):
        ...
        :return:
        """
        if isinstance(goid, int):
            goid = GOID.from_raw(goid)
        elif isinstance(goid, np.ndarray):
            goid = GOID.from_array(goid)
        else:
...
class GOID:
    ...
    def from_array(cls, arr):
        ...
        if isinstance(arr, np.ndarray):
            if arr.dtype == np.dtype('uint'):
                bitmap = ''.join([bin(v)[2:].rjust(_COMP_BITDEPTH, '0') for v in arr])
                return cls.from_raw(int(bitmap, 2))
        raise NotImplementedError
