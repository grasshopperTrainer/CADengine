...
class MainWindow(Window):
    def __init__(self):
        ...
        # create cursor
        self.devices.cursors.appendnew_cursor(self.devices.panes[1].pos)

        def fix_pos(glfww, xpos, ypos, mouse, pane, cursor):
            cursor.pos_global = tuple(map(lambda a: a/2, pane.glyph.size))

        self.devices.cursors[1].set_cursor_pos_callback(fix_pos, pane=self.devices.panes[1])
